 ...
 * @typedef {{ relative: string, info: import('node:fs').Stats, children?: string[] }} Entry
...
/** @type {WeakMap<CleanPlan, { entries: Entry[], rootInfo: import('node:fs').Stats }>} */
...
export async function removeCleanPlan(plan, { onProgress } = {}) {
  ...
  const checkRoot = async () => {
    const info = await lstat(plan.root)
  ...
  for (const entry of entries) {
    ...
    if (
      ...
      info.mtimeMs !== entry.info.mtimeMs ||
      info.ctimeMs !== entry.info.ctimeMs
    ) {
  ...
  try {
    ...
    for (const entry of entries.toReversed()) {
      ...
      const unlinked = removedLinks.get(identity) ?? 0
      // Unlink altera o ctime dos hard links restantes, mas não o conteúdo ou o mtime.
      if (
        !sameIdentity(info, entry.info) ||
        (!info.isDirectory() &&
          (info.nlink !== entry.info.nlink - unlinked ||
            info.size !== entry.info.size ||
            info.mtimeMs !== entry.info.mtimeMs ||
            (unlinked === 0 && info.ctimeMs !== entry.info.ctimeMs)))
      )
        throw new Error(`O alvo foi alterado: ${entry.relative}`)
      const absolute = path.join(plan.root, entry.relative)
      if (info.isDirectory()) await rmdir(absolute)
      else {
        await unlink(absolute)
        if (entry.info.nlink > 1) removedLinks.set(identity, unlinked + 1)
      }
      removed++
      onProgress?.(removed)
    }
